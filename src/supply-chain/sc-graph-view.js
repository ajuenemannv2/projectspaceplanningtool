(function(){
    ...
    function buildStyle() {
        return [
            { selector: 'node', style: {
                ...
                'label': 'data(label)', 'font-size': '11px', 'font-family': 'Inter, -apple-system, sans-serif',
                'color': '#1e293b', 'text-valign': 'bottom', 'text-halign': 'center',
                'text-margin-y': '4px', 'text-max-width': '90px', 'text-wrap': 'ellipsis',
                'border-width': '2px', 'border-color': 'data(borderColor)', 'border-opacity': 0.6,
                'transition-property': 'opacity, border-width', 'transition-duration': '200ms'
            }},
            { selector: 'node:selected', style: { 'border-width': '3px', 'border-color': '#2563eb', 'border-opacity': 1 }},
            ...
            { selector: 'edge.sole-source', style: { 'line-style': 'dashed', 'line-dash-pattern': [6, 3] }},
    ...
    function load(entities, relationships, scores) {
        ...
        entities.forEach(function(entity) {
            ...
            elements.push({ group: 'nodes', data: {
                id: entity.id, entityId: entity.id, label: entity.name,
                color: color, size: size,
                borderColor: isSole ? '#7c3aed' : '#94a3b8',
                structural: sc.structural, trajectory: sc.trajectory,
                sector: entity.industry_sector || '', type: entity.entity_type || '', country: entity.country_code || ''
            }});
        });
        ...
        relationships.forEach(function(rel) {
            ...
            elements.push({ group: 'edges', data: {
                id: rel.id, source: rel.source_entity_id, target: rel.target_entity_id,
                width: width, edgeColor: edgeColor, criticality: crit
            }, classes: rel.is_sole_source ? 'sole-source' : '' });
        });
    ...
    function runLayout() {
        if (!cy) return;
        var opts = typeof cola !== 'undefined'
    ...
    function applyFilters(filterState, scores) {
        ...
        cy.batch(function() {
            cy.nodes().forEach(function(node) {
                ...
                if (filterState.search) { if (!(d.label || '').toLowerCase().includes(filterState.search)) visible = false; }
            ...
            cy.edges().forEach(function(edge) {
                ...
                edge.toggleClass('dimmed', src.hasClass('dimmed') || tgt.hasClass('dimmed'));
            });
        });
    }
    ...
    function exportPNG() { return cy ? cy.png({ full: true, scale: 2, bg: '#ffffff' }) : null; }

    window.SCGraphView = { init: init, load: load, applyFilters: applyFilters, focusNode: focusNode, resetView: resetView, exportPNG: exportPNG };
})();
