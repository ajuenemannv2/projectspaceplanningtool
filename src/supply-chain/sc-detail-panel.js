(function(){
    ...
    function renderDetail(detail) {
        ...
        var trajPct = Math.round(((sc.trajectory + 100) / 200) * 100);
        html += '<div class="sc-detail-section">'
            ...
            + '<div class="sc-score-bar-wrap sc-score-bar-wrap--traj"><div class="sc-score-bar-center"></div><div class="sc-score-bar sc-score-bar--traj" style="width:' + Math.abs(sc.trajectory / 2) + '%;' + (sc.trajectory >= 0 ? 'left:50%;background:' + trajColor : 'right:50%;background:' + trajColor) + '"></div></div>'
        ...
        html += '<div class="sc-detail-section">'
            ...
            + kv('Revenue', entity.annual_revenue_usd ? '$' + formatRevenue(entity.annual_revenue_usd) : '—')
            + '</div>'
        ...
        if (policies.length || bills.length) {
            html += '<div class="sc-detail-section">'
                + '<div class="sc-detail-section-title">Policy Exposure</div>';
            ...
            policies.forEach(function(pi) {
                ...
                var pcolor = pdir === 'tailwind' ? '#10b981' : pdir === 'headwind' ? '#ef4444' : '#94a3b8';
                html += '<div class="sc-policy-item">'
                    + '<span class="sc-policy-icon" style="color:' + pcolor + '">' + icon + '</span>'
                    + '<div class="sc-policy-body">'
                    + '<div class="sc-policy-title">' + escapeHtml(pol.title || 'Policy') + '</div>'
            ...
            bills.forEach(function(bi) {
                ...
                var pcolor = bdir === 'tailwind' ? '#10b981' : bdir === 'headwind' ? '#ef4444' : '#94a3b8';
                var lobbyRatio = (bill.lobbying_for_usd || 0) + (bill.lobbying_against_usd || 0) > 0
                    ? Math.round((bill.lobbying_for_usd || 0) / ((bill.lobbying_for_usd || 0) + (bill.lobbying_against_usd || 0)) * 100)
                    : null;
                html += '<div class="sc-policy-item sc-bill-item">'
                    + '<span class="sc-policy-icon" style="color:' + pcolor + '">' + icon + '</span>'
                    + '<div class="sc-policy-body">'
                    + '<div class="sc-policy-title">' + escapeHtml(bill.title || 'Bill') + '</div>'
                    + '<div class="sc-policy-meta">'
                    + '<span class="sc-badge sc-badge--stage">' + escapeHtml((bill.stage || '').replace(/_/g,' ')) + '</span>'
                    + (lobbyRatio !== null ? '<span class="sc-lobby-ratio">' + lobbyRatio + '% industry for</span>' : '')
                    + '</div>'
        ...
        if (outbound.length || inbound.length) {
            html += '<div class="sc-detail-section">'
                + '<div class="sc-detail-section-title">Supply Relationships</div>';
