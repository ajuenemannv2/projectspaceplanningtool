(function(){
    ...
    function loadData(showError) {
        ...
        window.SCData.fetchAll().then(function(data) {
            ...
            _scores = window.SCScoring.computeAll(
                data.entities, data.relationships, data.policyImpacts, data.sectorTrends
            );
            window._scScoresCache = _scores;

            window.SCFilterPanel.build(document.getElementById('scFilterPanel'), data.entities);
            ...
            if (loadingEl) loadingEl.classList.add('sc-hidden');
            if (showError) showError('Failed to load supply chain data.', function(){ loadData(showError); });
        });
    }
    ...
    function exportGraph() {
        ...
        a.href = png; a.download = 'supply-chain-' + new Date().toISOString().slice(0,10) + '.png'; a.click();
    }
