(function(){
    ...
    function load(entities, scores) {
        ...
        entities.forEach(function(entity) {
            ...
            var marker = L.circleMarker([entity.lat, entity.lng], {
                radius: radius, fillColor: color, color: '#ffffff',
            ...
            heatPoints.push([entity.lat, entity.lng, Math.max(0.1, (sc.structural + Math.max(0, sc.trajectory)) / 200)]);
        });
    ...
    function applyFilters(filterState, scores) {
        ...
        Object.keys(markerMap).forEach(function(entityId) {
            ...
            if (filterState.search && !(ent.name || '').toLowerCase().includes(filterState.search)) visible = false;
            if (visible) { if (!markersLayer.hasLayer(marker)) markersLayer.addLayer(marker); }
    ...
    function focusEntity(entityId) {
        if (!map) return;
        var marker = markerMap[entityId];
        if (marker) { map.flyTo(marker.getLatLng(), 6, { duration: 0.8 }); marker.openTooltip(); }
    }
    ...
    window.SCMapView = { init: init, load: load, applyFilters: applyFilters, focusEntity: focusEntity, toggleHeatmap: toggleHeatmap, invalidateSize: invalidateSize };
})();
